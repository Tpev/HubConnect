<div class="max-w-3xl mx-auto p-6 space-y-6">
    ...
    <div class="flex items-center gap-2 text-sm">
        <div class="flex-1 h-1 rounded bg-slate-200">
            <div class="h-1 rounded bg-indigo-600" style="width: {{ $step >= 1 ? ($step==1? '33%' : ($step==2?'66%':'100%')) : '0%' }}"></div>
        </div>
    ...
    @if ($step === 1)
        <div class="bg-white rounded-xl shadow p-5 space-y-4">
            <div>
                ...
                <input type="text" wire:model.defer="headline" class="mt-1 w-full rounded-md border-slate-300" placeholder="e.g., Medical Sales Rep — Orthopedics & Spine">
            ...
            <div>
                ...
                <input type="text" wire:model.defer="location" class="mt-1 w-full rounded-md border-slate-300" placeholder="City, State / Region">
    ...
    @if ($step === 2)
        <div class="bg-white rounded-xl shadow p-5 space-y-4">
            <div>
                ...
                <input type="number" min="0" max="60" wire:model.defer="years_experience" class="mt-1 w-full rounded-md border-slate-300" placeholder="e.g., 5">
            ...
            <div>
                ...
                <textarea wire:model.defer="bio" rows="5" class="mt-1 w-full rounded-md border-slate-300" placeholder="Brief summary of your background, specialties, and achievements."></textarea>
                ...
                <div class="space-y-2">
                    @forelse ($skills as $i => $skill)
                        <div class="flex gap-2">
                            <input type="text" wire:model.defer="skills.{{ $i }}" class="flex-1 rounded-md border-slate-300" placeholder="e.g., Territory management">
            ...
            <div>
                ...
                @if ($profile?->cv_path)
                    <p class="text-sm text-slate-600 mt-1">Current file: <a class="text-indigo-600 underline" href="{{ Storage::disk('public')->url($profile->cv_path) }}" target="_blank">Open</a></p>
                @endif
            </div>
